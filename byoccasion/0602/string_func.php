...
//explode egy stringet bont egy szperaátor mentén és tömbkétn visszadja, ha adunk meg limitet akkor a maradékot egy utolsó elemként
    ...
    //limit használa
    /*$str14='1990-09-22-22-55-3-2';
    print_r(explode('-',$str14,4)).'<br>';*/

    /*$str14='1990-09-22';
    $separator='-';
    $return_tomb=array();
    $tmp_elem='';
    for ($i=0; $i < mb_strlen($str14); $i++) { 
        echo $str14[$i].'<br>';
        $char=$str14[$i];
    ...
    //saját explode, paraméterek string amit bontunk, $sep->separator ami mentén bontjuk a stringet
    /*function sajat_explode($string,$sep) {
        //létrehozzuk a változókat amikkel dolgozunk
        $return_tomb=array();
        $tmp_elem='';

        //karakterenként végig megyünk a tömbön
        for ($i=0; $i < mb_strlen($string); $i++) {
            //a jobb áttekinthetőség miatt
            $char=$string[$i];

            //echo 'i:'.$i."char: <br>";
            //ha nem $sep akkor hozzáadjuk az uj elemhez
            if ($char!=$sep ) {
                $tmp_elem .=$char;

            } else { //ha separator akkor hozzáadjuk a visszatérési tömbhöz és üritjuk az ideiglenes gyűjtőnket
                $return_tomb[]=$tmp_elem;
                $tmp_elem='';
            }
       
        }
        //az utolsó elemet hozzáadjuk mert nem lesz már separátor ami hozzáadná az else ágban
        $return_tomb[]=$tmp_elem;
